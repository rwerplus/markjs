 ...
 * @Desc: 鼠标样式插件
 */
export default function mousePlugin(instance, selfPen) {
  let _resolve = null;
  ...
  function showPen() {
    ...
    const base64Cursor = opt.penImg.startsWith("data:")
      ? opt.penImg
      : `data:image/svg+xml;base64,${btoa(opt.penImg)}`;
    canvasEle.style.cursor = `url("${base64Cursor}") 0 12, default`;
  }
  ...
  instance.on("HOVER-ITEM", (item, curItem, allInItems, e, inPointIndex) => {
    if (!curItem || curItem.isClosePath) {
      isHoverInItem = true;
      setElePos(e.clientX + 12, e.clientY + 15);
